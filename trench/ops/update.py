"""Knowing a newer Trench exists, and — where it is safe — installing it.
...
path:
  ...
  * **It does not restart itself by default.** Applying an update stages new
    ...
    memory. Restarting is the supervisor's job, and Trench will only ask
...
#: Ceiling on a downloaded artifact. A Trench wheel is a couple of MB; this
...
def _normalize(name: str) -> str:
    """PEP 503 normalisation, so `Trench_DNS` and `trench-dns` are one name."""
...
class Install:
    """How this Trench got onto the box, and whether it may rewrite itself."""
...
def detect_install(package_dir: Path | None = None) -> Install:
    ...
    if sys.prefix == sys.base_prefix:
        return Install("unknown", prefix, python, False,
                       "not running inside a virtual environment Trench owns")
...
class Updater:
    ...
    async def check(self) -> Release | None:
        ...
        if release is None:
            ...
            log.info("Trench %s is available (running %s)", release.version, self.current)
        self.state.save(self.state_path)
        return release
    ...
    async def _apply_locked(self, release: Release | None, version: str | None,
        ...
        try:
            ...
            await self._smoke_test(wheel, staging, release)
        ...
        if self._audit is not None:
            ...
            if asyncio.iscoroutine(noted):
                await noted
        log.warning("Trench %s installed; the running process is still %s — "
    ...
    async def _smoke_test(self, wheel: Path, staging: Path, release: Release) -> None:
        """Install into a throwaway environment and make it prove what it is.
    ...
    async def _maybe_restart(self) -> None:
        ...
        if self.cfg.restart != "systemd":
            return
        unit = self.cfg.unit or "trench"
        if shutil.which("systemd-run"):
            argv = ["systemd-run", "--collect", "--on-active=2",
                    f"--unit=trench-update-restart-{int(time.time())}",
        ...
        try:
            ...
            # Privilege is the usual cause: Trench sheds root after binding,
