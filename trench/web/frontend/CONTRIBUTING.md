...
which is generated from `trench/api/settings.py`.
...
- Build check: `npm run build` (includes vue-tsc). Dev: `npm run dev` with a
  trench backend on 127.0.0.1:8089 (`python3 -m trench --config …`).
- Backend API surface: `trench/api/server.py` (REST + `/api/v1/ws`).
