...
_BUILTINS = {
    "dns64": "trench.plugins.builtin.dns64:Dns64Plugin",
    "block_tld": "trench.plugins.builtin.block_tld:BlockTldPlugin",
}
...
def _load_class(ref: str) -> type[Plugin]:
    ...
    `python -m trench` the working directory is `sys.path[0]`, so that second
    ...
    if ref in _BUILTINS:
        ...
        raise ValueError(
            ...
            f"TRENCH_ALLOW_EXTERNAL_PLUGINS=1 is set (known: "
...
def _allow_external() -> bool:
    import os
    return os.environ.get("TRENCH_ALLOW_EXTERNAL_PLUGINS", "") == "1"
