...
from trench.stats.counters import LATENCY_BOUNDS_US
...
def test_rcode_and_upstream_labels_are_exported():
    ...
    text = render(c)
    assert 'trench_query_rcodes_total{rcode="SERVFAIL"} 1' in text
    assert 'trench_upstream_answers_total{upstream="1.1.1.1:53"} 1' in text
    assert "trench_clients_seen 2" in text
...
def test_latency_histogram_is_cumulative_and_totals_agree():
    ...
    text = render(c)
    buckets = {}
    for line in series(text, "trench_query_duration_seconds_bucket"):
    ...
    assert "trench_query_duration_seconds_count 4" in text
...
def test_label_values_are_escaped():
    ...
    line = series(render(c), "trench_upstream_answers_total")[0]
